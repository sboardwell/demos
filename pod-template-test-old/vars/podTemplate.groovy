def call(def args, Closure body) {
  ...
  def possibleMatches = []
  def logStr = []
  getInfos(inheritFrom, possibleMatches, logStr)
  // check results
...
@NonCPS
def getInfos(String inheritFrom, def possibleMatches, def logStr) {
    ...
    Label.get(inheritFrom)?.getClouds().each { cloud ->
      ...
      possibleMatches.add([cloud: cloud.name, inheritFrom: inheritFrom, defaultContainer: defaultContainer])
    }
}
